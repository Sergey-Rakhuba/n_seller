...
import "../../card/card.css"; // Reuse existing card styles for now

export default function SellerCard({ ad, onSelectAd }) {
    return (
        <div className="card_block" onClick={() => onSelectAd && onSelectAd(ad)}>
            <img src={ad?.image || "/users/9dc9049feb4a79dfd5f930926e7ac9b20a3e0a55.png"} alt={ad?.title || "Ad image"} />
            <div className="card_content">
                <p className='card_itm-group'>{ad?.location || "123 Main St, Anytown, USA"}</p>
                <h3 className='card_itm-name'>{ad?.title || "Beautiful Family House"}</h3>
                <div className='card_itm-block_params'>
                    {ad?.year && <span>{ad.year} год выпуска</span>}
                    {ad?.weight && <span>{ad.weight} вес</span>}
                    {ad?.mileage && <span>{ad.mileage} mileage</span>}
                </div>
            </div>
            <br />
            <div className="card_btn-block">
                <div className="adress_item flex_row flex_btw align-ctr">
                    <div className="location flex_row flex_btw align-ctr">
